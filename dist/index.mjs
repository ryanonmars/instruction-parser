...
function getPriceInUSDByMint(tokenMint) {
  return __async(this, null, function* () {
    try {
      ...
      let payload = yield ky.get(`https://price.jup.ag/v4/price?ids=${tokenMint}`).json();
...
function getTokenMap() {
  return __async(this, null, function* () {
    const response = yield ky.get("https://token.jup.ag/all").json();
...
function extract(signature, connection, tx, blockTime) {
  return __async(this, null, function* () {
    ...
    const volumeInUSD = outAmountInUSD && inAmountInUSD ? Decimal2.min(outAmountInUSD, inAmountInUSD) : outAmountInUSD != null ? outAmountInUSD : inAmountInUSD;
    const swap = {};
    ...
    if (exactOutAmount) {
      ...
      if (outAmountInUSD) {
        swap.exactOutAmountInUSD = new Decimal2(exactOutAmount).div(outAmount.toString()).mul(outAmountInUSD).toNumber();
      }
    }
    ...
    if (exactInAmount) {
      ...
      if (inAmountInUSD) {
        swap.exactInAmountInUSD = new Decimal2(exactInAmount).div(inAmount.toString()).mul(inAmountInUSD).toNumber();
      }
    }
